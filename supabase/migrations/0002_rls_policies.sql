...
create policy biz_update on businesses for update using (id = current_business_id() and current_role() = 'ADMIN');
...
create policy prof_write on profiles for update
  using (business_id = current_business_id() and current_role() = 'ADMIN')
  with check (business_id = current_business_id() and current_role() = 'ADMIN');
...
create policy tx_select on transactions for select using (
  ...
  and (current_role() in ('ADMIN','CONTADOR') or user_id = (select auth.uid()))
);
...
create policy tx_update on transactions for update using (
  business_id = current_business_id() and current_role() = 'ADMIN'
);
...
create policy budgets_select on budgets for select using (
  business_id = current_business_id() and current_role() in ('ADMIN','CONTADOR')
);
