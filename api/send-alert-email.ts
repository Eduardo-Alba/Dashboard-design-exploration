...
// Envia siempre al correo del propio usuario autenticado, nunca a un `to` del cliente —
// evita que este endpoint se use como relay para mandar correos a terceros.
export default async function handler(req: Request): Promise<Response> {
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({ from: 'FinanZen <onboarding@resend.dev>', to: caller.user.email, subject, text: message }),
  })
  if (!res.ok) {
